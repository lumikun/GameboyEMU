...
bool window_visible()
{
    return LCDC_WIN_ENABLE && lcd_get_ctx()->win_x >= 0 && lcd_get_ctx()->win_x <= 166 && lcd_get_ctx()->win_y >= 0 && lcd_get_ctx()->win_y < YRES;
}
...
void pixel_fifo_push(u32 val)
{
    ...
    if (!ppu_get_ctx()->pfc.pixel_fifo.head) {
        ppu_get_ctx()->pfc.pixel_fifo.head = ppu_get_ctx()->pfc.pixel_fifo.tail = next;
    } else {
...
u32 fetch_sprite_pixels(int bit, u32 color, u8 bg_color)
{
    for (int i = 0; i < ppu_get_ctx()->fetched_entry_count; i++) {
        int sp_x = (ppu_get_ctx()->fetched_entries[i].x - 8) + ((lcd_get_ctx()->scroll_x % 8));
        ...
        u8 lo = !!(ppu_get_ctx()->pfc.fetch_entry_data[(i * 2) + 1] & (1 << bit)) << 1;
        ...
        if (!bg_priority || bg_color == 0) {
            color = (ppu_get_ctx()->fetched_entries[i].f_pn) ?
                lcd_get_ctx()->sp2_colors[hi|lo] : lcd_get_ctx()->sp1_colors[hi|lo];
            if (hi|lo)
                break;
        }
    }
...
void pipeline_load_sprite_tile() {
    ...
    while(le) {
        ...
        if ((sp_x >= ppu_get_ctx()->pfc.fetch_x && sp_x < ppu_get_ctx()->pfc.fetch_x + 8) ||
            ((sp_x + 8) >= ppu_get_ctx()->pfc.fetch_x && (sp_x + 8) < ppu_get_ctx()->pfc.fetch_x + 8))
            ppu_get_ctx()->fetched_entries[ppu_get_ctx()->fetched_entry_count++] = le->entry;
        le = le->next;
...
void pipeline_load_sprite_data(u8 offset) {
    ...
    for (int i = 0; i < ppu_get_ctx()->fetched_entry_count; i++) {
        ...
        if (sprite_height == 16)
            tile_index &= ~(1);
        ppu_get_ctx()->pfc.fetch_entry_data[(i * 2) + offset] = bus_read(0x8000 + (tile_index * 16) + ty + offset);
    }
}
...
void pipeline_load_window_tile()
{
    ...
    if (ppu_get_ctx()->pfc.fetch_x + 7 >= lcd_get_ctx()->win_x && ppu_get_ctx()->pfc.fetch_x + 7 < lcd_get_ctx()->win_x + YRES + 14) {
        if (lcd_get_ctx()->ly >= window_y && lcd_get_ctx()->ly < window_y + XRES) {
            u8 w_tile_y = ppu_get_ctx()->window_line / 8;
            ppu_get_ctx()->pfc.bgw_fetch_data[0] = bus_read(LCDC_WIN_MAP_AREA +
...
void pipeline_fetch()
{
    switch(ppu_get_ctx()->pfc.cur_fetch_state) {
        case FS_TILE: {
            ...
            if (LCDC_BGW_ENABLE) {
                ppu_get_ctx()->pfc.bgw_fetch_data[0] = bus_read(LCDC_BG_MAP_AREA +
                                                                (ppu_get_ctx()->pfc.map_x / 8) +
        ...
        case FS_DATA0: {
            ppu_get_ctx()->pfc.bgw_fetch_data[1] = bus_read(LCDC_BGW_DATA_AREA +
                (ppu_get_ctx()->pfc.bgw_fetch_data[0] * 16) +
                ppu_get_ctx()->pfc.tile_y);
            pipeline_load_sprite_data(0);
            ppu_get_ctx()->pfc.cur_fetch_state = FS_DATA1;
        } break;
        case FS_DATA1: {
            ppu_get_ctx()->pfc.bgw_fetch_data[2] = bus_read(LCDC_BGW_DATA_AREA +
                (ppu_get_ctx()->pfc.bgw_fetch_data[0] * 16) +
...
void pipeline_push_pixel()
{
    if (ppu_get_ctx()->pfc.pixel_fifo.size > 8) {
        ...
        if (ppu_get_ctx()->pfc.line_x >= (lcd_get_ctx()->scroll_x % 8)) {
            ppu_get_ctx()->vbuffer[ppu_get_ctx()->pfc.pushed_x +
                (lcd_get_ctx()->ly * XRES)] = pixel_data;
...
{
    ppu_get_ctx()->pfc.map_y = (lcd_get_ctx()->ly + lcd_get_ctx()->scroll_y);
    ppu_get_ctx()->pfc.map_x = (ppu_get_ctx()->pfc.fetch_x + lcd_get_ctx()->scroll_x);
    ppu_get_ctx()->pfc.tile_y = ((lcd_get_ctx()->ly + lcd_get_ctx()->scroll_y) % 8) * 2;
